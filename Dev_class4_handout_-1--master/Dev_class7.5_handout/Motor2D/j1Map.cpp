...
void j1Map::PropagateBFS()
{
	...
	if (frontier.Count() > 0)
	{
		p2Queue_item<iPoint>* last = frontier.start;
		iPoint north, east, south, west;
		north = { last->data.x, last->data.y - 1 };
		east = { last->data.x + 1, last->data.y };
		south = { last->data.x, last->data.y + 1 };
		west = { last->data.x - 1, last->data.y };
...
bool j1Map::IsWalkable(int x, int y) const
{
	...
	for (; item != NULL; item= item->next)
	{
		...
		{
			return { (x>0 && x < data.width) && (y>0 && y < data.height) && layer->Get(x,y) == 0 };
		}
	}
	return true;
}
