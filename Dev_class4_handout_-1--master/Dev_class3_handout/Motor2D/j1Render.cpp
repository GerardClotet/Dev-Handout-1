...
bool j1Render::Load(const pugi::xml_node& state) {
	camera.x = state.child("camera").attribute("x").as_int();
	camera.y = state.child("camera").attribute("y").as_int();
	return true;
}
...
{
	pugi::xml_node camera_node = state.append_child("camera");
