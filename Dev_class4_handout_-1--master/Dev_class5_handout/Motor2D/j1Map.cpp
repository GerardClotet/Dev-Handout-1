	...
	while (item_layer != nullptr)
	{
		for (uint row = 0; row < item_layer->data->width; row++)
		{
			for (uint column = 0; column < item_layer->data->height; column++) {
				...
				SDL_Rect tile = item_tileset->data->GetTileRect(item_layer->data->tiles[Get(row, column)]);
...
bool j1Map::Load(const char* file_name)
{
	...
	for (layer = map_file.child("map").child("layer"); layer && ret; layer.next_sibling("layer")) {
		map_layer* newlayer = new map_layer();
...
bool j1Map::LoadLayer(pugi::xml_node& node, map_layer* layer) {
	...
	memset(layer->tiles, 0, layer->width*layer->height * sizeof(uint));  //sizeof->num of bytes thing occupies in memory 
